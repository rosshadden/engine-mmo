		...
		.defines({
			...
			checkHit:	function(obj){
				...
				if(obj.posY - obj.hsizeY < 0){
					res.hitY = 1;
					res.posY = obj.hsizeY;
				}else if(obj.posY + obj.hsizeY > engine.sys.sizeY){
					res.hitY = 1;
					res.posY = engine.sys.sizeY - obj.hsizeY;
				}
				...
				}else if(obj.posX + obj.hsizeX > engine.sys.sizeX){
					res.hitX = 1;
					this.trigger('score:right');
				}
			
				//check hits on paddles
				engine('paddle').each(function(i){
					if(i.hit(obj)){
						res.hitX = 1;
